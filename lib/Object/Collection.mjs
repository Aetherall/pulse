...
class State {
	...
	toJSON() {
		const data = {}
		for (const attr of this.operations.keys()) {
			const op = this.operations.get(attr).toJSON()
			if (op instanceof NoOp) continue
			data[attr] = this.operations.get(attr).toJSON()
		}
		return data
	}
}
...
export default class Collection {
	constructor(core, pointer) {
		...
		this.core = core
		this.pointer = pointer || core.cache.pointer.create(this)
		this.state = new State(this)
		this.core.cache.object.register(this)
	}

	get id() {
		return this.pointer.id || this.pointer.localId
	}
