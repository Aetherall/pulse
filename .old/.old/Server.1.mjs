	...
	parseOperation() {
		if (this.op === 'one') {
			if (this.operation.connect) {
				this.connect = Pointer.fromJson(this.operation.connect)
			} else if (this.operation.disconnect) {
				this.disconnect = Pointer.fromJson(this.operation.disconnect)
			} else {
				throw new Error('[ Operation ] Malformed Operation - One OP should have a connect or a disconnect')
			}
		} else if (this.op === 'many') {
			if (this.operation.add) {
				this.add = this.operation.add.map(toAdd => Pointer.fromJson(toAdd))
			}
			if (this.operation.remove) {
				this.add = this.operation.remove.map(toRemove => Pointer.fromJson(toRemove))
			}
		} else {
			if (this.operation.value) this.value = this.operation.value
		}
	}

	parseDependencies() {
		const deps = new PulseSet()
		if (this.value) return deps
		if (this.connect && !this.connect.exists) return deps.add(this.connect)
		if (this.disconnect && !this.disconnect.exists) return deps.add(this.disconnect)
		if (this.add) for (const item of this.add) if (!item.exists) deps.add(item)
		if (this.remove) for (const item of this.remove) if (!item.exists) deps.add(item)
		return deps
	}
}
